...
批处理系统没有太多的用户操作，在该系统中，**调度算法目标是保证吞吐量(单位时间内处理客户请求数量)和周转时间（从提交到终止的时间）。**
...
* 同步与互斥
  * 同步：多个进程因为合作产生的直接制约关系，使得进程有一定的先后执行关系（串行）。
...
进程通信是一种手段，而进程同步是一种目的。也可以**为了能够达到进程同步的目的，需要让进程进行通信，传输一些进程同步所需要的信息。**

#### 进程间通信(IPC)的两种主要形式

* 共享内存
  * 共享内存模型会建立起一块**供协作进程共享的内存区域**，进程通过向此共享区域读出或写入数据来交换信息;(通过内存映射文件实现，将共享内存区域与文件相关联)
  * 优点：共享内存系统只在建立共享内存区域时需要系统调用；一旦建立共享内存区域，所有的访问都可作为常规内存访问，无需借助内核，无需在进程间来回拷贝信息；
  * 缺点：**多处理核系统会涉及高速缓存一致性问题，这是由[共享数据在多个高速缓存之间迁移]而引起的**
* 消息传递
  * 消息传递模型通过在协作进程间**交换消息**实现通信
  * 优点：**消息传递对于交换较少数量的数据很有用，因为无需避免冲突，对于分布式系统，消息传递比共享内存更易于实现**
  * 缺点：消息传递的实现经常需要系统调用，因此需要消耗更多时间以便内核介入，并且信息会在进程间来回拷贝

## 1. 管道

管道是通过**调用 pipe 函数创建的，fd[0] 用于读，fd[1] 用于写。**
...
它是一个计数器，用于**为多个进程提供对共享数据对象的访问。**

## 5. 共享内存

**共享内存会有[高速缓存一致性]问题，这是由于共享数据在多个高速缓存之间迁移而引起的**
...
* 多个进程可以**将同一个文件映射到它们的地址空间从而实现共享内存**。另外 XSI 共享内存不是使用文件，而是使用内存的匿名段。
